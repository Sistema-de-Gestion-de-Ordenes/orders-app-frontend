...
export class DriverService {
    async createDriver(data: DriverData): Promise<void> {
        const url     = `${API_CONFIG.BASE_URL}${API_CONFIG.ENDPOINTS.DRIVERS}`;
        ...
        if (response.statusCode < 200 || response.statusCode >= 300) {
            throw new Error(raw?.error ?? raw?.message ?? 'Error al crear el repartidor.');
        }
    }
    ...
    async getDriversCount(): Promise<number> {
        ...
        if (response.statusCode >= 200 && response.statusCode < 300) {
            const drivers = response.content?.toJSON?.() ?? [];
